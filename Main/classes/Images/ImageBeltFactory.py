...
class ImageBeltFactory:
    ...
    @staticmethod
    def getManyImagesBelt(img_paths: list, x: int, y: int, velocity: int, edge: int, horizontal: bool = True, bottom_left=False):
        ...
        for i in range(1, len(img_paths)):
            ...
            if horizontal:
                x_previous = image_w_list[i-1].image_rect.right
                image_w_list[i].set_position(x_previous, y, bottom_left)
            else:
                y_previous = image_w_list[i-1].image_rect.bottom
                image_w_list[i].set_position(x, y_previous, bottom_left)
        return ImageBelt(image_w_list, velocity, edge, horizontal)
